...
db.connect((err) => {
  if (err) {
    console.error('❌ [Alerta] Erro ao conectar ao MySQL:', err);
    return;
  }
  db.query('CREATE DATABASE IF NOT EXISTS markcamp_db', (err) => {
    if (err) {
      console.error('❌ [Alerta] Erro ao criar banco:', err);
      return;
    }
    db.changeUser({ database: 'markcamp_db' }, (err) => {
      if (err) {
        console.error('❌ [Alerta] Erro ao selecionar banco:', err);
        return;
      }
      console.log('✅ [Alerta] Conexão com markcamp_db estabelecida.');
      dbReady = true;
    });
  });
});
...
const transporter = nodemailer.createTransport({
    ...
    port: 587,
    auth: {
        user: 'salma.schamberger@ethereal.email',
        pass: '5s181Utj6gJD2vXR96'
    }
});
...
async function verificarEnviarAlertas() {
  if (!dbReady) {
    console.log('[Alerta] Banco ainda não está pronto, tentando novamente em 2 segundos...');
    setTimeout(verificarEnviarAlertas, 2000);
    return;
  }
  ...
  try {
    ...
    db.query(query, async (err, rows) => {
      if (err) {
        console.error('[Alerta] Erro SQL:', err);
        return;
      }

      if (rows.length === 0) {
        console.log('[Alerta] Nenhum item em falta.');
        return;
      }
      ...
      for (const [email, itens] of Object.entries(agrupado)) {
        try {
          ...
          console.error(`[Alerta] Erro ao enviar para ${email}:`, err.message);
        }
      }
    });
  } catch (error) {
    console.error('[Alerta] Erro geral:', error);
  }
}

function iniciarVerificacaoEstoque(intervaloMs = 1 * 60 * 5000) {
  console.log(`[Alerta] Verificação de estoque iniciada (${intervaloMs / 60000} min).`);
  // Primeira execução: aguarda a conexão estar pronta (a função já verifica dbReady)
